...
namespace p3109
{
    ...
    struct binary
    {
        ...
        static constexpr std::uint64_t neginf_codepoint = (Sigma == Signed) ? (pow2_u64(K) - 1) : 0; // 0 for unsigned: never used
        ...
        static constexpr bool isnan(const binary &x)
        {
            return x.codepoint == nan_codepoint;
        }
        ...
        static constexpr bool isposinf(const binary &x)
        {
            return x.codepoint == posinf_codepoint;
        }
        ...
        constexpr bool isnan() const { return codepoint == nan_codepoint; }
        constexpr bool isinf() const
        {
            if constexpr (Sigma == Signed)
                return codepoint == posinf_codepoint || codepoint == neginf_codepoint;
            else
                return codepoint == posinf_codepoint;
        }
        constexpr bool isposinf() const { return codepoint == posinf_codepoint; }
