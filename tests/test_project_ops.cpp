...
template <unsigned K, unsigned P, p3109::Signedness Sigma, p3109::Domain Delta> struct TestProjectOps
{
    ...
    static bool test_saturate_ovf_inf_toward_zero()
    {
        ...
        return test_utils::expect_equal(
            z, max_finite, "OvfInf + TowardZero should clamp positive overflow to max finite");
    }
    ...
    static bool test_project_nan()
    {
        ...
        return test_utils::expect_true(
            z.codepoint == p3109::pow2_u64(K - 1), "Project(NaN) should return NaN codepoint");
    }
    ...
    static bool test_project_ovf_inf_nearest()
    {
        ...
        return test_utils::expect_true(z.codepoint == (p3109::pow2_u64(K - 1) - 1),
            "Project OvfInf + nearest should map overflow to +inf codepoint");
    }
    ...
    static bool test_project_ovf_inf_toward_zero()
    {
        using Format = p3109::binary<K, P, Sigma, Delta>;
        const auto z =
            p3109::Project<Format, p3109::TowardZero>(p3109::mpfr_float("1e100"), p3109::SaturationMode::OvfInf);
        return test_utils::expect_true(z.codepoint == (p3109::pow2_u64(K - 1) - 2),
            "Project OvfInf + TowardZero should clamp to max finite codepoint");
    }
...
int main()
{
    ...
    TestProjectOps<8, 3, p3109::Signed, p3109::Extended>::run(s);
