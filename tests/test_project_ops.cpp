...
struct TestProjectOps
{
    ...
    static void run()
    {
        ...
        run_test({float_type::name(), "round_ties_to_even"}, test_round_ties_to_even());
        run_test({float_type::name(), "round_ties_to_away"}, test_round_ties_to_away());
        run_test({float_type::name(), "saturate_finite"}, test_saturate_finite());
        run_test({float_type::name(), "saturate_propagate"}, test_saturate_propagate());
        run_test({float_type::name(), "saturate_ovf_inf_toward_zero"}, test_saturate_ovf_inf_toward_zero());
        run_test({float_type::name(), "saturate_ovf_inf_nearest"}, test_saturate_ovf_inf_nearest());
        run_test({float_type::name(), "project_nan"}, test_project_nan());
        run_test({float_type::name(), "project_ovf_inf_nearest"}, test_project_ovf_inf_nearest());
        run_test({float_type::name(), "project_ovf_inf_toward_zero"}, test_project_ovf_inf_toward_zero());
        run_test({float_type::name(), "project_finite_requires_satfinite"}, test_project_finite_requires_satfinite());
    }
};

int main()
{
    test_utils::init("project_ops");

    TestProjectOps<8, 3, p3109::Signed, p3109::Extended>::run();
